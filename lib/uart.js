...
Uart.prototype.setopts = function(options) {
	...
	if (options.data_bits) {
		opts.data_par += (options.data_bits == 9)? 1 : 0;
	}
	if (options.parity_bit) {
		opts.data_par += (options.parity_bit == 'E')? 1 :
	...
	var baudcmd = bauds[opts.baudrate] || 0x64,  // baud defaults to 9600
...
/**
 * Enable RX echoing.  Disabled by default so that rec codes aren't corrupted
...
Uart.prototype.echo_rx = function(on, callback) {
	...
	this.bp.wait_for_data(0x01, function(err) {
		if (!err) {
			self.bp.log('uart', 'Started receiving RX');
			self.echo_rx_on = on;
