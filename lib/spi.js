...
/**
 * Write and read bytes of data from SPI bus. TODO: test this
 * @param  {number}   num      Number of bytes to read
 * @param  {Function} callback Execute this when done
 */
Spi.prototype.write_read = function(write, callback) {
	var self = this,
		lenbyte = 0x10 + write.length - 1;

	// TODO: block SPI operation while a read/write is in progress

	if (write.length > 16) {
		return new Error('Can only read between 0 and 16 bytes');
	}
	if (!this.started) {
